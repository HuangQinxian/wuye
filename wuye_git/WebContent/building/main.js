$(document).ready(function(){
	...
	$.getJSON("area/getAll.mvc",function(data){
		for(var i = 0; i < data.length; i++)
		$("select#areaSelect").append("<option value='"+data[i].no+"'>"+data[i].name+"</option>")
	});
	
	$.getJSON("buildingtype/getAll.mvc",function(data){
		for(var i = 0; i < data.length; i++){
	...
	$("input#codeSelect").on("blur",function(){
		code = $(this).val();
		$("#buildingGrid").jqGrid('setGridParam',{postData:{areaNo:areaNo,buildingTypeNo:buildingTypeNo,code:code}}).trigger("reloadGrid");
	});
	
	$("#buildingGrid").jqGrid({
		url: 'building/getListByAllWithPage.mvc',
		datatype: "json",
		mtype:"GET",
		styleUI : 'Bootstrap',
		colModel: [
			{ label: '建筑编码', name: 'code', width: 150 },
